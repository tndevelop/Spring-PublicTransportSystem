...
class JwtAuthenticationTokenFilter: OncePerRequestFilter() {
    ...
    private fun getAuthentication(token: String, req: HttpServletRequest): UsernamePasswordAuthenticationToken? {
        try {
            ...
            val roles = userMap.get("roles") as ArrayList<String>
            for (role in roles) {
                authorities.add(SimpleGrantedAuthority(role))
            }
            val auths = userMap.get("authorities") as ArrayList<String>
            for (auth in auths) {
                authorities.add(SimpleGrantedAuthority(auth))
            }

            val userDetails = userDetailsService.loadUserByUsername(userMap.get("username") as String)
